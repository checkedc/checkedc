...
extern array_ptr<int> h : bounds(f - 2, f + 3) rel_align(char);
...
extern void f5(void) {
  ...
  auto int arr5 checked[5] : bounds(arr5, arr5 + 5) rel_align(char);
  auto int arr6 checked[5] : bounds(arr5, arr5 + len) rel_align(char);
...
extern void f6(int *arr checked[] : count(5)) {
  ...
  int ((*arr6 checked[5])(int, int)) : bounds(arr5, arr5 + len) rel_align(char);
  // Checked array of checked pointers to functions
  ptr<int (int, int)> arr7 checked[5] : count(5);
  ptr<int (int, int)> arr8 checked[5] : bounds(arr8, arr8 + 5) rel_align(char);
  ...
  array_ptr<ptr<int (int, int)>> w : bounds(v, v + 5) rel_align(char) = 0;
  array_ptr<ptr<int (int len, array_ptr<int> arr : count(len))>> x : bounds(x, x + 5) rel_align(char) = 0;
}
