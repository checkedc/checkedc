...
\item No variable used in \var{e} can be modified between the \texttt{suspends}
and \texttt{holds} declarations.
\item No variable used in \var{e} can have its address taken.  This is to avoid
...
bounds fits nicely into these constraints for supporting
...
predicates that use bounds expression to describe the initialization
...
if we want to express that \var{x} the first n elements of are initialized
and the remaining elements are not, we could have
\texttt{\var{x} : init\_data(x, x + n)
\&\& \var{x} : uninit\_data(x + n + 1, x + len)}.
