...
\item We introduce functions{\em IsDisjoint} and {\em IsDisjointLValue} that determines when 
two pointer-typed expressions or two lvalue expressions do not alias.  It return true when two 
pointers must point to disjoint ranges of memory that are accessible or two lvalue expressions must 
denote disjoint ranges of memory. Otherwise it return false. This is used to detect when there are 
conflicting reads/write and writes/writes involving lvalue expressions.

These functions are pessimistic: they assume aliasing unless aliasing can be disproven.  Currently, 
Checked C have no notation for describing the lack of aliasing in data structures or during function calls.   
The functions could be made less pessimistic with this information.
...
with a bounds declaration, the programmer is responsible for ensuring that bounds declaration invariant
is true.  The programmer must ensure that after the assignment, the bounds declaration invariant
holds for the object before any member involved in the bounds declaration is read.
Checking of bounds declaration will not enforce that member bounds invariants hold after 
those assignments.  An example  \lstinline|ptr<int> p = ... ; *p = ...|,
...
To check anything about bounds declarations involving pointer dereferences, we must make some assumptions.  In code that is entirely checked, these assumptions will always be met.  However,
...
same pointer is passed for both arguments). We assume that the lvalue \code{dest->len1} does not overlap 
with the lvalue \code{src->len2} or \code{src->buf}.
