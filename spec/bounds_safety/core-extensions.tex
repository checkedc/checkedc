...
void decode(array\_ptr<char> output\_buffer, array\_ptr<char> input\_buffer, 
            size\_t input_len\textit{, size\_t output_len})
\{
    ...
    while (src < src\_bound) \{
        switch (*current++) \{
            case UNCOMPRESSED\_BYTES: \{ 
                ...
                src += 2;
                \textit{// need check that dst and src have at least len bytes of}
                \textit{// space}
                memcpy(dst, src, len);
                src += len;
                dst += len;                
...
\{
      ...
            case UNCOMPRESSED\_BYTES: \{
                size\_t len = src[0] + src[1]*256;
                src += 2;
                \textit{// check that dst and src have at least len bytes of}
                \textit{// space}
                \textit{if (dst + len >= dst\_bound || src + len >= src\_bound) \{}
                \textit{   goto failure;}
                \textit{\}}
                memcpy(dst, src, len);
                src += len;
            ...
            size\_t input\_len\textit{, size\_t output\_len})
{
      ...
            case UNCOMPRESSED\_BYTES: { 
                size\_t len = src[0] + src[1]*256;
                src += 2;
                \textit{dynamic\_check(dst + len < dst\_bound && src + len < src_bound);}
                memcpy(dst, src, len);
                src += len;
                dst += len;                
                break;
...
\{ 
    for (size\_t i = 0; i < src\_count; i++) \{
        if (\textit{src[i]} == marker) \{
           break;
        \}
        \textit{dst[i]} = \textit{src[i]};
    \}
\}
\end{alltt}
...
void append(array\_ptr<char> dst: count(dest\_count), 
            ...
            size\_t dst\_count, size\_t src\_count)
\{ 
    \textit{dynamic\_check(src\_count <= dst\_count);}
    for (size\_t i = 0; i < src\_count; i++) \{
        if (src[i] == marker) \{
            break;
        \}
    ...
    if (src\_count <= dst\_count) \{
        ...
        for (size\_t i = 0; i < src\_count; i++) \{
            if (src[i] == marker) \{
                break;
            \}
            \textit{dst[i]} = src[i];  // bounds check needed
        \}   
    \}
\}
\end{alltt}
