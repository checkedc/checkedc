...
where \var{relop} is one of \texttt{<}, \texttt{<=}, \texttt{==},
\texttt{!=}, \texttt{>=}, or texttt{>}. 
Recall that range-expressions are a
...
rules used to check facts do not include the identities 
...
To determine whether an expression \var{e} is invertible, the compiler uses
the facts that are true before the expression to prove that
\texttt{MININT >= \var{e}} and \texttt{\var{e} <= MAXINT}.
