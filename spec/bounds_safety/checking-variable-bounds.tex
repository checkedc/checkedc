...
\subsection{Introducing temporaries}

Sometimes we need to discuss bounds that are given in terms of the value
...
We use {\em temporaries} to denote the values of expressions or subexpressions
in these cases.   A temporary is a compiler-introduced temporary variable.
The scope and lifetime of a temporary is the entire full expression where the
temporary  is used.   If the full expression is an initializer or part of an initializer,
the scope and lifetime extend to the declarator being initialized.  All temporaries
are typed, and their address cannot be taken.  Temporaries must always
be assigned to first before being used.

Temporaries have a different behavior for assignments than source-level
variables.  When a temporary variable is assigned to, the assignment happens
immediately after the right-hand side expression has been evaluated.  This differs from
a source-level variable, whose assignment may happen at some less-precisely
specified time.

Sometimes the bounds for an expression may depend on a variable
that is modified by the expression.  This is a problem if the expression
is then used to access memory: the bounds check depends on a variable
that has been modified by an expression.  Given the semantics of
C for variable assignment, the variable is an indeterminate state:
the assignment may or may have been completed.  A case where this can happen
is when using pre or post-increment variables:
\begin{lstlisting}
array_ptr<int> current : bounds(current, end) = ...
// current has bounds that depend on itself, so current++ makes
// bounds(current, end) indeterminate at the * operation.
while (current < end && *current++ != sentinel) {
 ...
}
\end{lstlisting}

We introduce temporaries for this case also,  so that bounds checks
are not using variables that are in an indeterminate state.

Here is a list of expressions where temporaries should be introduced to hold
the results of the expressions.
\begin{itemize}
\item Function calls, where the called functions have return bounds
that use  the return values.
\item String or array literal expressions.
\item Compound assignments to variables used in bound expressions.
\item Pre-or post-increment/decrements to variables used in bounds declarations.
\item Call arguments that are modifying expressions, where the corresponding
parameters are used in the return bounds expressions for the called functions.
all argument that is a modifying expression and
\end{itemize}

We introduce a special syntactic form to represent the initialization of a temporary:
\lstinline|init-temporary| $t$, $e$ initializes a temporary $t$ to $e$.
...
A temporary was introduced for the result of the literal expression.
...
is used, a temporary variable was introduced to hold the result of the
...
Given an expression of the form \code{++}\var{e1}, \code{--}\var{e1},
 ...
 If $e1$ appears in its own bounds, the temporary introduced to hold
 the result value of $e1$ should be used in the bounds in place of
 $e1$.   For post-increment/decrement operators, the temporary can
 be used in place of $e1$ in the bounds.  For pre-increment operators,
 the temporary - 1 should be used in place of $e1$.  For pre-decrement operators,
 the temporary + 1 should be used in place of $e1$. 
...
assignment of $v$.
\item Let $B = RB$. 
\item If $e$ is an element of a set of equivalent expressions $F$ in $EQ$, let $G = F - PV$.
...
\item If $e$ is a non-modifying expression, let $G  = \{ Val \}$.
...
\item Otherwise, try to construct a version $Val^\prime$ of $Val$ that computes the same value 
      ...
      use an expression from $G_i $(or $G_1$ when $Form = PostIncDec$).
      If $G_i$ is empty, $Val^\prime$ cannot be constructed.
      ...
      The same invariant applies to the $B_i$.
...
one for each arm. This in turn would require that $Check$ take a set of contexts as
