...
\item Otherwise, try to construct a version $Val^\prime$ of $Val$ that computes the same value 
      ...
      Note that an invariant about each $G_i$ is that it will not contain any uses of variables in $PV_i$ or $PV$.
      This means that assignments to variables in other subexpressions (the $S_k$) or expressions whose
      evaluation is unordered with respect to $S_i$ to do not affect the value of $G_i$.  This is a 
      consequence of the check at 
