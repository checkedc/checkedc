...
The function $Check(e, C, EQ, PV, BES, EES, Form, FB)$ takes as inputs:
\begin{enumerate}
...
\item A stack of bounds expressions $BES$ that need to be updated if any variables in those bounds
expressions are modified.
\item A stack of sets of equivalent expressions $EES$ that also need to be updated if any variables
in those expressions are modified.
...
\item An updated stack of bounds expressions $UBES$.
\item An updated stack of equivalent expressions $UEES$.
...
The function $CheckLValue(e, C, EQ, PV, BES, EES)$ is similar to $Check$.  It takes the
same first 6 arguments as $Check$, except that $e$ evaluates to an lvalue.
It returns:
\begin{enumerate}
...
\begin{enumerate}
\item Let $(B_{lv}, RB, \mathit{UC}, UEQ, G_{lv}, UBES, UEES) = CheckLValue(e, C, EQ, PV, BES, EES)$.
...
\item Let $(B_{lhs}, RB, \mathit{UC}_{lhs}, {UEQ}_{lhs}, \_, {UBES}_{lhs}, {UEES}_{lhs}) = CheckLValue(lhs, C, EQ, P_{lhs}, BES, EES)$.  
\item Let $(B_{rhs},  \mathit{UC}_{rhs}, {UEQ}_{rhs}, G_{rhs}, {UBES}_{rhs}, {UEES}_{rhs}) = 
Check(rhs, \mathit{UC}_{lhs}, {UEQ}_{lhs},P_{rhs}, {UBES}_{lhs}, {UEES}_{lhs}, Form, RB)$.
...
\item Otherwise, if $UEQ_{rhs}$ contains a set of equivalent expressions $F$ that contains $v$ as an element,
and $F$ contains a variable $w \neq v$, set $iv$ to $w$.
\item Otherwise, let $iv = $ {\it None}.
\end{enumerate}
\item Let $(B, \mathit{UC}, UEQ, G, UBES, UEES) = Update(v, iv, B_{rhs}, \mathit{UC}_{rhs}, UEQ_{rhs}, G_{rhs})$.
\end{enumerate}
