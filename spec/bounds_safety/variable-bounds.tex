...
It suggested that programmers use simple non-modifying expressions because
...
the pointer and its bounds and bytes. Given 
...
We first define the set of flow-sensitive bounds declarations that apply to a component of a function.  A component is an expression statement,
variable declaration, or part of a compound statement. For any
flow-sensitive bounds
...
The runtime check becomes \texttt{e1 <= e1 < e1 + e2}.
In this example, the check becomes \texttt{x <= x <
x + c}. The condition \texttt{x <= x} is trivially true. The
condition \texttt{x < x + c} simplifies to \texttt{0
< c}, that is \texttt{c \textgreater{} 0}, which is what one
would expect.
...
bounds declarations in more detail and explains why that was not chosen.
