...
\var{t} <= \var{e3}}.   For memory writes, assignment of 0 (the null value)
using the assignment operator is allowed at exactly the upper bound.
The upper bound is the beginning of the sequence and there must be enough
space in the sequence for a null terminator.  Otherwise, the check
is the same as for \arrayptr.   Here is a precise description:
\begin{itemize}
