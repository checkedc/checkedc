...
\subsection{From unchecked pointer to checked pointer type}
...
\subsection{From checked pointer to unchecked pointer type}
...
\subsection{Between checked pointer types}

An expression with a type ``checked pointer to \var{T}'' can be converted implicitly
to a different kind of ``checked pointer to \var{T}''.  The different kind
must be \arrayptr\ or \ptr. The expression must occur as the right-hand
side of an assignment or a call argument. The left-hand side of the
assignment or the parameter type must have the different kind of checked pointer
type.   Implicit conversions to \ntarrayptrT\ are not allowed because of the possibility
of null terminators being overwritten via aliases to the location of the null terminators.

An expression with a type ``checked pointer to \var{T}'' can be converted implicitly to
a ``checked pointer to \texttt{void}''.  The expression
must occur as the right-hand side of an assignment, a call argument,
or an arm of a conditional expression.  The type of the left-hand side of the assignment,
the parameter, or the other arm of the conditional expression
must be the checked \void\ pointer type.   For example, implicit conversions from
\ptrinst{\var{T}} to \ptrvoid\ and from \arrayptrinst{\var{T}} to \arrayptrvoid\ are
allowed  at assignments.
