...
the function declaration with an \lstinline+itype(for_any(...))+ clause.
...
\end{lstlisting}
The \lstinline+itype(for_any(T))+ indicates that \lstinline+bsearch+ has a generic
...
applied to type variables, the type variable is ignored and any
interface types that use type variables are discarded.
