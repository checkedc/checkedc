...
\section{Support for \arrayptr\ and \arrayview\ pointers that are not relatively aligned}
...
\begin{longtable}[c]{p{0.8in}p{1.4in}p{1.4in}p{1.4in}}
\toprule
Set of program points & By-value & By-reference & General comments\tabularnewline
\midrule
\endhead
Direct & & & By-value and by-reference evaluation is indistinguishable.
Because there are no assignments, both produce the same
result\tabularnewline \midrule
...
for variable. &\tabularnewline \midrule
Intermediate & Introduces hidden temporary variables that hold bounds
value.

May lead to runtime failures if the lhs variable is assigned a pointer
to an object outside of the declared bounds. & &\tabularnewline \midrule
